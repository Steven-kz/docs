import{_ as t,M as s,p as r,q as d,R as n,t as c,N as e,a1 as a}from"./framework-5866ffd3.js";const p={},u=a(`<h1 id="mysql进阶" tabindex="-1"><a class="header-anchor" href="#mysql进阶" aria-hidden="true">#</a> Mysql进阶</h1><h2 id="一、mysql进阶" tabindex="-1"><a class="header-anchor" href="#一、mysql进阶" aria-hidden="true">#</a> 一、Mysql进阶</h2><h3 id="_1-1mysql基础" tabindex="-1"><a class="header-anchor" href="#_1-1mysql基础" aria-hidden="true">#</a> 1.1Mysql基础</h3><p>DBMS：数据库管理系统 Mysql数据库的数据类型<br> 1.数字类型：int bigint tinyint double decimal(⻓度，⼩数点的位数)<br> 2.字符串类型：char varchar text<br> 3.⽇期类型：date datetime<br> Mysql约束条件：为字段添加约束条件<br> 1.主键约束 2.外键约束<br> 3.⾮空约束<br> 4.唯⼀约束<br> 5.⾃增约束<br> 6.默认约束<br> SQL：结构化查询语⾔，操作数据库<br> SQL分类：<br> 1.DDL:数据定义 create drop alter<br> 2.DML:数据操作 insert update delete<br> 3.DQL:数据查询 select<br> 4.TPL(TCL):事务处理 commit rollback<br> 5.DCL:数据控制语⾔ grant<br> 6.CCL:指针控制语⾔ cursor</p><h4 id="_7大语句" tabindex="-1"><a class="header-anchor" href="#_7大语句" aria-hidden="true">#</a> 7⼤语句：</h4><p>1.创建表：CREATE table 表名 (字段 数据类型 约束条件,……)<br> 2.修改表：ALTER TABLE 表名 ADD 字段名 数据类型 约束条件<br> 3.删除表：DROP TABLE 表名<br> 4.新增数据：INSERT INTO 表名(字段名,……) values(值,……)<br> 5.修改数据：UPDATE 表名 set 字段=值,…… [where 条件]<br> 6.删除数据：DELETE FROM 表名 [where 条件]<br> 7.查询数据：SELECT 字段|* FROM 表名 [where 条件]<br> 查询条件：and or in like between and not is null order by limit group by having<br> 聚合函数：avg max min count sum<br> 复杂查询：⼦查询、笛卡尔积查询（⾃然查询）、内连接、左外连接、右外连接、全连接（Mysql不⽀ 持）<br> 需要刷题：https://www.nowcoder.com/exam/oj?page=1&amp;tab=SQL%E7%AF%87&amp;topicId=268<br> 数据删除：drop（把表都删除）、delete（删除表中数据）、truncate（删除表中的数据，并删除⾃增 器） 单位：分 bigint 好处：避免四舍五⼊ 缺陷：⻚⾯需要处理<br> 单位：元 decimal 好处：单位元，不需要额外处理 缺陷：四舍五⼊</p><h3 id="_1-2-视图" tabindex="-1"><a class="header-anchor" href="#_1-2-视图" aria-hidden="true">#</a> 1.2 视图</h3><p>视图：对查询语句（临时表）的封装，简化操作，操作视图跟操作表⼀样<br> 创建视图：</p><div class="language-mysql line-numbers-mode" data-ext="mysql"><pre class="language-mysql"><code>create view 视图名 as 查询语句; 
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>删除触发器：drop trigger 触发器名称; 测试触发器：满⾜条件</p><div class="language-mysql line-numbers-mode" data-ext="mysql"><pre class="language-mysql"><code>-- 触发器：满足指定的条件（1.时间 before after 2.操作⽅式 新增、修改、删除）
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>索引的分类：<br> **1.主键索引 ** **2.普通索引 ** **3.唯⼀索引 ** <strong>4.联合索引（复合索引）</strong><br><strong>没有索引之前：</strong><br><img src="https://cdn.staticaly.com/gh/Steven-kz/BlogImgs@master/imgaes/20230311002735.png" alt="image.png"><strong>创建索引之后：</strong><br><img src="https://cdn.staticaly.com/gh/Steven-kz/BlogImgs@master/imgaes/20230311002943.png" alt="image.png"> 由于数据不多，所以性能提升并不明显 <strong>查看是否存在索引：</strong><br><img src="https://cdn.staticaly.com/gh/Steven-kz/BlogImgs@master/imgaes/20230311003034.png" alt="1"> explain的结果说明：<br> type所显示的是查询使⽤了哪种类型，type包含的类型包括如下图所示的⼏种，从好到差依次是<br> **system &gt; const &gt; eq_ref &gt; ref &gt; range &gt; index &gt; all ** <strong>查看表上的索引：</strong></p><div class="language-mysql line-numbers-mode" data-ext="mysql"><pre class="language-mysql"><code>show index from t_pcall
...
11.limit 
分⻚ 
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,47),o=n("br",null,null,-1),v=n("strong",null,"笛卡尔积乘积指在数学上，两个",-1),m={href:"https://so.csdn.net/so/search?q=%E9%9B%86%E5%90%88&spm=1001.2101.3001.7020",target:"_blank",rel:"noopener noreferrer"},b=n("strong",null,"集合",-1),k=n("strong",null,"X和Y的笛卡尔积 (Cartesian product)，又称直积，表示 X x Y，第一个对象是 X 的成员而第二个对象是 Y 的所有可能有序对的其中一个成员。",-1),g=a(`<div class="language-mysql line-numbers-mode" data-ext="mysql"><pre class="language-mysql"><code>eg1:
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>SQL优化三部曲：定位、分析、解决开发中遇到问题解决三部曲： <strong>定位（⽇志、debug）</strong><strong>分析（根据异常，和定位的点，去分析代码）</strong><strong>解决（根据分析结果，因果而异）</strong></p><h3 id="_2-2-sql优化之定位" tabindex="-1"><a class="header-anchor" href="#_2-2-sql优化之定位" aria-hidden="true">#</a> 2.2 SQL优化之定位</h3><p>定位：找到执⾏慢的sql语句<br> ⽅式，任选其⼀：<br> 1.Mysql的慢查询⽇志<br> 在mysql的配置⽂件中：windows:my.ini linux:my.cnf <img src="https://cdn.staticaly.com/gh/Steven-kz/BlogImgs@master/imgaes/20230311003244.png" alt="image.png"> 每次只需要，查看slow.log⽇志⽂件，记录就是执⾏超过2秒的sql语句<br> 2.Spring AOP实现查询语句的记录<br> aop的环绕通知，去记录查询的sql语句的执⾏时间，如果执⾏超过2秒，就记录sql语句<br> 3.Druid的SQL监控<br> 配置Druid的SQL监控，获取执⾏慢的SQL语句<br> 4.借助⼯具，软件实现<br> 阿⾥云-Mysql等等</p><h3 id="_2-3-sql优化之分析" tabindex="-1"><a class="header-anchor" href="#_2-3-sql优化之分析" aria-hidden="true">#</a> 2.3 SQL优化之分析</h3><p>分析：根据执⾏慢的SQL语句，进⾏分析，找到慢的原因<br> 汇总⼀些常⻅：<br> 1.SQL语句有业务逻辑，影响性能<br> 2.SQL语句，联合查询的多表关系不是最短路径<br> 3.SQL语句索引失效<br> 4.SQL语句查询条件的顺序不对<br> 5.Mysql并发量过⼤<br> 6.SQL语句对应的表的数据量太⼤<br> 7.SQL语句太⻓<br><strong>8.SQL语句查询条件函数计算</strong></p><h3 id="_2-4-sql优化之解决" tabindex="-1"><a class="header-anchor" href="#_2-4-sql优化之解决" aria-hidden="true">#</a> 2.4 SQL优化之解决</h3><p>解决：对号⼊座，尝试处理<br> 1.SQL语句有业务逻辑，影响性能<br> 拆分业务逻辑，可以把逻辑放到Java代码中解决，改SQL语句<br> 2.SQL语句，联合查询的多表关系不是最短路径<br> 分析涉及到的多表关系，查看能不能找到最短路径，⼲掉没⽤的表连接<br> 3.SQL语句索引失效<br> 想办法让索引⽣效，⽐如合理设计索引，采⽤联合索引，使⽤最左前缀原则，索引的数量、索引的字段<br> 4.SQL语句查询条件的顺序不对<br> on先执⾏，再执⾏where,最后执⾏having<br> 所以可以把有些查询条件放到on那⾥进⾏筛选<br> 5.Mysql并发量过⼤<br> 2种⽅案：1.舍弃连接，数据库连接池，设置最⼤连接<br> 2.搭建Mysql的集群（多主多从）-中间件解决 （Mycat、Sharding-Jdbc）<br> 6.SQL语句对应的表的数据量太⼤<br> 实现数据分⽚，采⽤第三⽅技术：Sharding-Jdbc<br> 7.SQL语句太⻓<br> 拆分为多个sql语句，在Java代码中整合数据<br> 8.SQL语句查询条件函数计算<br> 避免在sql的查询条件中使⽤函数进⾏计算处理，可以考虑Java代码中实现数据处理<br> 结合项⽬模块</p><h3 id="_2-5-常用的sql优化方案" tabindex="-1"><a class="header-anchor" href="#_2-5-常用的sql优化方案" aria-hidden="true">#</a> 2.5 常⽤的SQL优化⽅案</h3><div class="language-mysql line-numbers-mode" data-ext="mysql"><pre class="language-mysql"><code>1、你必须选择记录条数最少的表作为基础表  
